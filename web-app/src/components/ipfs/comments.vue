          ...
          <div class="flex flex-1 min-w-0 relative items-center">
            ...
            <div class="flex-1 mx-4">
              <p class="flex text-sm text-gray-500 truncate">
                ...
                <UseTimeAgo v-slot="{ timeAgo }" :time="item.created" v-if="item.created">
                  {{ timeAgo }}
                </UseTimeAgo>
                <eos-icons:loading class="h-6 text-black w-6" v-else />
              </p>
              <p class="font-medium text-sm text-gray-900 break-words">{{ item.comment }}</p>
            </div>
